...
@Serializable
data class Session(
    val id: Int, val capacity: Int, val date: String, val game: Int, val closed: Boolean, val players: List<Int>
)

fun Session.checkIfCapacityCanBeUpdated(capacity: Int) =
    (capacity > this.capacity) || (!closed && players.size <= capacity)
...
fun validateSessionUpdate(session: Session, date: String, capacity: Int, pid: Int) {
    if (!session.checkIfPlayerIsOwner(pid))
        throw AppException.PlayerCantUpdateSession("Player $pid can't update session ${session.id}")
